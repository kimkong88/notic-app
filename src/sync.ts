...
async function withRetry<T>(fn: () => Promise<T>, attempts: number = SYNC_RETRY_ATTEMPTS): Promise<T> {
  let lastErr: unknown
...
async function setLastPullAt(db: NoticDB, partition: string, epochMs: number): Promise<void> {
...
export async function setSyncPaused(db: NoticDB, value: boolean): Promise<void> {
...
export async function checkServerNewer(db: NoticDB): Promise<boolean> {
  ...
  try {
    ...
    if (lastPullAt > 0 && data.lastUpdatedAt > lastPullAt + SERVER_NEWER_GRACE_MS) {
...
function buildLocalPayload(): {
  ...
  const workspaces: PullResponse['workspaces'] =
    workspacesList.length > 0
      ...
      : [{ id: DEFAULT_WORKSPACE_ID, name: 'Workspace 1', isDefault: true, updatedAt: Date.now() }]

  return { notes, folders, workspaces }
}

/** Merge local + server. Only entities present in local are kept (so local deletes are not resurrected). Server overwrites when newer. */
function computeMergedState(
  ...
  // Workspaces: only include workspaces present in local (server overwrites when newer). Don't resurrect server-only workspaces after delete.
  ...
  for (const sw of serverWs) {
    ...
    if (!localW) continue
    const localTs = localW.updatedAt ?? 0
    if (sw.updatedAt > localTs) {
      workspaceById.set(sw.id, sw)
    } else {
      // Preserve local color/icon if server doesn't have them (matches extension)
  ...
  // Folders: only include folders present in local (server overwrites when same id). Don't resurrect server-only folders (e.g. after workspace/folder delete).
  ...
  for (const f of server.folders) {
    if (folderById.has(f.id)) {
      ...
      folderById.set(f.id, localF) // local overwrites server (matches extension)
    }
  }
  const folders = Array.from(folderById.values())

  // Notes: only include notes present in local (server overwrites when newer). Don't resurrect server-only notes (e.g. after workspace/note delete).
  ...
  for (const n of server.notes) {
    ...
    if (existing && n.lastModified > existing.lastModified) noteById.set(n.id, n)
  }
...
function formatChangePart(c: { key: string; oldValue: unknown; newValue: unknown }): string {
...
function entityDisplayLabel(entityType: 'workspace' | 'folder' | 'note', _fallbackId: string): string {
  switch (entityType) {
    case 'note':
...
function noteDisplayLabel(note: { displayName?: string; content?: string; id: string }): string {
  const isIdLike = (s: string) =>
    /^[0-9a-f-]{36}$/i.test(s) || s.length > 40 || /^(clip_|session_)/i.test(s)
  if (note.displayName && !isIdLike(note.displayName)) return note.displayName
...
function formatOverwriteMessage(
  ...
  const isLikelyUuid = (s: string) => /^[0-9a-f-]{36}$/i.test(s) || s.length > 40
  const name =
    displayName && !isLikelyUuid(displayName) ? displayName : entityDisplayLabel(entityType, displayName)
  const summary = parts.length === 1 ? parts[0] : parts.join('. ')
  const entityLabel = entityType === 'note' ? 'Note' : entityType === 'folder' ? 'Folder' : 'Workspace'
  return `${entityLabel} "${name}" — ${summary}`
}

type LocalPayload = { notes: PullResponse['notes']; folders: PullResponse['folders']; workspaces: PullResponse['workspaces'] }

/** Compute "server overwrote local" log entries. Matches extension computeServerOverwriteLogEntries. */
function computeServerOverwriteLogEntries(local: LocalPayload, server: PullResponse): SyncLogEntry[] {
  const now = Date.now()
  ...
  for (const sw of server.workspaces) {
    ...
    if (changed.length > 0)
      entries.push({
        ...
        message: formatOverwriteMessage('workspace', lw.name ?? sw.id, changed),
        details: { entityType: 'workspace', entityId: sw.id, changed },
      })
  }
  const folderKeys = ['name', 'parentId', 'displayName', 'workspaceId', 'color']
  for (const sf of server.folders) {
    ...
    if (changed.length > 0)
      entries.push({
        at: now,
        kind: 'server_overwrote_local',
        message: formatOverwriteMessage('folder', lf.name ?? sf.id, changed),
        details: { entityType: 'folder', entityId: sf.id, changed },
      })
  }
  const noteKeys = ['content', 'lastModified', 'displayName', 'folderId', 'workspaceId', 'deletedAt', 'color']
  for (const sn of server.notes) {
    ...
    if (changed.length > 0)
      entries.push({
        at: now,
        kind: 'server_overwrote_local',
        message: formatOverwriteMessage('note', noteDisplayLabel(ln), changed),
...
function computeDeletedIds(
  server: PullResponse,
  local: LocalPayload
): { deletedNoteIds: string[]; deletedFolderIds: string[]; deletedWorkspaceIds: string[] } {
  ...
  return {
    deletedNoteIds: server.notes.filter((n) => !localNoteIds.has(n.id)).map((n) => n.id),
    deletedFolderIds: server.folders.filter((f) => !localFolderIds.has(f.id)).map((f) => f.id),
    deletedWorkspaceIds: server.workspaces.filter((w) => !localWorkspaceIds.has(w.id)).map((w) => w.id),
  }
}

/** Build delta payload: local-only + local-newer entities + deleted*Ids. Matches extension buildDeltaPayload. */
function buildDeltaPayload(local: LocalPayload, server: PullResponse): PushBody {
...
function applyDeltaToSnapshot(
  ...
  const noteById = new Map(server.notes.filter((n) => !delNotes.has(n.id)).map((n) => [n.id, n]))
  for (const n of payload.notes) noteById.set(n.id, n)
  const folderById = new Map(server.folders.filter((f) => !delFolders.has(f.id)).map((f) => [f.id, f]))
  for (const f of payload.folders) folderById.set(f.id, f)
  const workspaceById = new Map(server.workspaces.filter((w) => !delWorkspaces.has(w.id)).map((w) => [w.id, w]))
  const workspaceList =
...
/** GET /sync (all pages). */
async function pullFromServer(db: NoticDB, since?: number): Promise<PullResponse> {
  ...
  do {
    ...
    if (!cursor && page.deletedNoteIds?.length) deletedNoteIds = page.deletedNoteIds
    if (!cursor && page.deletedFolderIds?.length) deletedFolderIds = page.deletedFolderIds
    if (!cursor && page.deletedWorkspaceIds?.length) deletedWorkspaceIds = page.deletedWorkspaceIds
  ...
  if (deletedWorkspaceIds?.length) out.deletedWorkspaceIds = deletedWorkspaceIds
  return out
}
...
async function pushPayload(db: NoticDB, body: PushBody): Promise<void> {
  const workspacesForApi = (body.workspaces.length > 0 ? body.workspaces : [{ id: DEFAULT_WORKSPACE_ID, name: 'Workspace 1', isDefault: true, updatedAt: Date.now() }]).map((w) => ({
    id: w.id,
    name: w.name,
    isDefault: w.isDefault,
...
async function mergeIntoDbAndStores(db: NoticDB, partition: string, merged: PullResponse): Promise<void> {
  const notesP: Array<NoteData & { partition: string }> = merged.notes.map((n) => {
    const title = (n.displayName ?? (n.content.split('\n')[0]?.trim().slice(0, 100) ?? '')) || 'Untitled'
  ...
  const foldersP: Array<Folder & { partition: string }> = merged.folders.map((f) => ({ ...f, partition }))
  const workspacesP: Array<WorkspaceInfo & { partition: string }> = merged.workspaces.map((w) => ({
    id: w.id,
    name: w.name,
    isDefault: w.isDefault,
...
export async function triggerFullSync(db: NoticDB, options?: TriggerFullSyncOptions): Promise<void> {
  ...
  if (!tokens) {
    await appendSyncChangeLog(db, [
      { at: Date.now(), kind: 'error', message: 'Full sync failed: not signed in' },
    ])
    setSyncState('failed')
    return
  }
  ...
  try {
    ...
    const successLog: SyncLogEntry[] = [
      {
        at: now(),
        kind: 'fully_synced',
        message: skipPush ? 'Pulled and merged (sync paused)' : 'Full sync succeeded',
      },
      ...overwriteEntries,
    ]
    ...
    await removeLocalKeysForDeletedIds(db, partition, server.deletedNoteIds ?? [], server.deletedFolderIds ?? [])
...
export async function triggerSync(db: NoticDB): Promise<void> {
  ...
  try {
    ...
    lastServerSnapshot =
      serverToUse != null ? applyDeltaToSnapshot(serverToUse, rawPayload) : localPayloadToPullResponse(full)
    await appendSyncChangeLog(db, [{ at: now(), kind: 'fully_synced', message: 'Sync succeeded' }])
    setSyncState('synced')
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error'
    await appendSyncChangeLog(db, [{ at: now(), kind: 'error', message: `Sync failed: ${message}` }])
    setSyncState('failed')
  }
}
