 ...
 */
export function decodeGoogleCredential(credential: string): GoogleUserProfile | null {
 ...
 */
export async function persistLastUser(db: NoticDB, profile: GoogleUserProfile): Promise<void> {
...
export async function fetchGoogleProfileFromToken(
  ...
  if (typeof data?.sub !== 'string' || typeof data?.name !== 'string' || typeof data?.picture !== 'string') {
    return null
  }
  return {
    sub: data.sub,
