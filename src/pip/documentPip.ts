...
export async function openPipWithNote(
  ...
  try {
    ...
    setPipIframeUrl(requestedWindow.document, isDarkMode, noteIds, activeId)
 ...
 */
function buildPipUrl(noteIds: string[], activeId: string | null, dark: boolean): string {
  const origin = typeof window !== 'undefined' ? window.location.origin : ''
  const darkStr = dark ? '1' : '0'
  const ids = noteIds.length ? noteIds.join(',') : ''
  const active = activeId ?? ''
  return `${origin}/pip?noteIds=${encodeURIComponent(ids)}&activeId=${encodeURIComponent(active)}&dark=${darkStr}`
}
...
export function setPipIframeUrl(
  ...
  const iframe = pipDocument.getElementById('notic-pip-iframe') as HTMLIFrameElement | null
  if (!iframe) return
 ...
 */
export function refreshPipWindowUrl(noteIds: string[], activeId: string | null): void {
