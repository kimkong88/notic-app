...
export function SlashCommandPlugin() {
  ...
  const restoreSelection = useCallback(() => {
    ...
    if (!saved) return
    const anchor = $createPoint(saved.anchorKey, saved.anchorOffset, saved.anchorType)
    const focus = $createPoint(saved.focusKey, saved.focusOffset, saved.focusType)
  ...
  const insertSlashAndHide = useCallback(() => {
    ...
    if (saved) {
      editor.update(() => {
        const ap = $createPoint(saved.anchorKey, saved.anchorOffset, saved.anchorType)
        const fp = $createPoint(saved.focusKey, saved.focusOffset, saved.focusType)
        const sel = $createRangeSelection()
  ...
  useEffect(() => {
    return mergeRegister(
      editor.registerCommand(
        KEY_DOWN_COMMAND,
        (event: KeyboardEvent) => {
          if (event?.key !== '/' || event.shiftKey || event.ctrlKey || event.metaKey) return false
          ...
          editor.getEditorState().read(() => {
            ...
            if ($isTextNode(node)) {
              if (anchorOffset === 0) {
                ...
                if (parent && $isParagraphNode(parent) && parent.getFirstChild() === node) {
                  atLineStart = true
                }
              }
            } else if ($isParagraphNode(node)) {
              if (node.getFirstChild() === null) atLineStart = true
            }
          })
  ...
  const stripButtons = [
    { title: 'Heading 1', Icon: Type, action: () => replaceBlockWithHeading(1) },
    { title: 'Heading 2', Icon: Heading2, action: () => replaceBlockWithHeading(2) },
    { title: 'Heading 3', Icon: Heading3, action: () => replaceBlockWithHeading(3) },
    ...
    { title: 'Bullet list', Icon: List, action: () => insertListType('bullet') },
    { title: 'Numbered list', Icon: ListOrdered, action: () => insertListType('number') },
    { title: 'Task list', Icon: ListTodo, action: () => insertListType('check') },
