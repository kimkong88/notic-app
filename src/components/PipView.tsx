...
function parsePipParams(): {
    noteIds: string[];
    activeId: string | null;
    dark: boolean;
} {
    if (typeof window === "undefined")
        return { noteIds: [], activeId: null, dark: false };
...
export function PipView() {
        ...
        (noteId: string, patch: Parameters<typeof updateNote>[1]) => {
            ...
            if (window.parent !== window) {
                window.parent.postMessage(
                    { type: "notic-pip-note-update", noteId, patch },
                    "*"
                );
            }
        },
    ...
    const [noteTitlesFromMain, setNoteTitlesFromMain] = useState<
        Record<string, string>
    >({});
    const [noteColorsFromMain, setNoteColorsFromMain] = useState<
        Record<string, string>
    >({});

    const effectiveActiveId = noteIds.includes(activeTabId ?? "")
        ? activeTabId
        : noteIds[0] ?? null;
    ...
    useEffect(() => {
        const handler = (e: MessageEvent) => {
            ...
            if (d && d.type === "notesUpdate" && Array.isArray(d.noteIds)) {
                ...
                if (
                    d.notePayloads != null &&
                    typeof d.notePayloads === "object"
                ) {
                    const updateNoteStore = useNotesStore.getState().updateNote;
                    d.noteIds.forEach((id: string) => {
                        const payload = d.notePayloads[id];
                        if (payload && typeof payload === "object")
                            updateNoteStore(id, payload);
                    });
                }
                setNoteIds(d.noteIds);
                setActiveTabId(d.activeId ?? d.noteIds[0] ?? null);
                if (d.noteTitles != null && typeof d.noteTitles === "object")
                    setNoteTitlesFromMain(d.noteTitles);
                if (d.noteColors != null && typeof d.noteColors === "object")
                    setNoteColorsFromMain(d.noteColors);
            }
    ...
    const handleSwitchTab = useCallback(
        (noteId: string) => {
            ...
            if (window.parent !== window) {
                window.parent.postMessage(
                    { type: "notic-pip-switch-tab", noteId },
                    "*"
                );
            }
        },
    ...
    const handleCloseTab = useCallback(
        (e: React.MouseEvent, noteId: string) => {
            ...
            if (window.parent !== window) {
                window.parent.postMessage(
                    { type: "notic-pip-close-tab", noteId, isEmpty },
                    "*"
                );
            }
        },
    ...
    const handleContentChange = useCallback(
        (noteId: string) => (newContent: string) => {
            ...
            contentTimeoutRef.current = setTimeout(() => {
                ...
                if (window.parent !== window) {
                    const latest =
                        useNotesStore.getState().notes[noteId]?.content;
                    if (latest !== undefined) {
                        window.parent.postMessage(
                            {
                                type: "notic-pip-note-update",
                                noteId,
                                patch: { content: latest },
                            },
                            "*"
                        );
                    }
        ...
        <div className="pip-container">
            <div className="pip-tabs">
                {sortedNoteIds.map((id, index) => {
                    ...
                    const showSep =
                        !isActive && !nextIsActive && nextId != null;
                    return (
                        <div
                            key={id}
                        ...
                        >
                            ...
                            {showSep && (
                                <div className="pip-tab-sep" aria-hidden />
                            )}
                        </div>
                    );
                })}
                ...
                {effectiveActiveId && (
                    <div className="note-container pip-tab-panel-active">
                        ...
                        {contentLength >= NOTE_CHAR_WARNING && (
                            <div
                                className="pip-note-char-warning visible"
                                aria-live="polite"
                            >
                                Approaching note limit (
                                {contentLength.toLocaleString()} /{" "}
                                {NOTE_CHAR_LIMIT.toLocaleString()} characters)
                            </div>
                        )}
                    </div>
                )}
            ...
            {contextMenu && (
                ...
                >
                    ...
                    <button
                        ...
                        className="pip-context-menu-item"
                        onClick={() => {
                            handleRenameOpen(contextMenu.noteId);
                            setContextMenu(null);
                        }}
                    >
                        Rename
                    ...
                    <button
                        ...
                        className="pip-context-menu-item"
                        onClick={() => {
                            handleCloseOthers(contextMenu.noteId);
                            setContextMenu(null);
                        }}
                    >
                        Close others
                    ...
                    <button
                        ...
                        className="pip-context-menu-item"
                        onClick={() => {
                            handleCloseAfter(contextMenu.noteId);
                            setContextMenu(null);
                        }}
                    >
                        Close after
                ...
                >
                    ...
                    >
                        <div className="pip-modal-header">
                            <h3 className="pip-modal-title" id="pipRenameTitle">
                                Rename note
                            </h3>
                            <input
                                type="text"
