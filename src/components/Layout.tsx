...
export function Layout() {
    ...
    const pipUnsupportedModalOpen = useUIStore(
        (s) => s.pipUnsupportedModalOpen
    );
    const setPipUnsupportedModalOpen = useUIStore(
        (s) => s.setPipUnsupportedModalOpen
    );
    const sessionExpiredModalOpen = useUIStore(
        (s) => s.sessionExpiredModalOpen
    );
    const setSessionExpiredModalOpen = useUIStore(
        (s) => s.setSessionExpiredModalOpen
    );
    ...
    const setServerNewerBannerVisible = useUIStore(
        (s) => s.setServerNewerBannerVisible
    );
    ...
    useEffect(() => {
        const unsub = subscribeServerNewer(() =>
    ...
    const handleMouseUp = useCallback(() => {
        ...
        if (finalWidth < SIDEBAR_COLLAPSE_THRESHOLD) {
            ...
            setSidebarWidth(
                Math.min(
                    SIDEBAR_WIDTH_MAX,
                    Math.max(SIDEBAR_WIDTH_MIN, finalWidth)
                )
            );
        }
    ...
    useEffect(() => {
        const handler = (event: MessageEvent) => {
            ...
            if (t === "notic-pip-note-update") {
                ...
                if (
                    fromPip &&
                    event.data?.noteId != null &&
                    event.data?.patch != null
                ) {
                    if (
                        useNotesStore.getState().notes[event.data.noteId] ==
                        null
                    )
                        return;
                    useNotesStore
                        .getState()
                        .updateNote(event.data.noteId, event.data.patch);
                }
                return;
            }
            if (
                t !== "notic-pip-add-note" &&
                t !== "notic-pip-close-tab" &&
                t !== "notic-pip-switch-tab" &&
                t !== "notic-pip-ready"
            )
                return;
            const pipWin = getPipWindow();
            ...
            if (t === "notic-pip-add-note") {
                ...
                sendNotesUpdateToPip(
                    after.openInPipNoteIds,
                    after.openInPipActiveNoteId,
                    { noteTitles, noteColors, notePayloads }
                );
                ...
                const notePayloadsClose: Record<
                    string,
                    {
                        content?: string;
                        title?: string;
                ...
                sendNotesUpdateToPip(
                    ...
                    after.openInPipActiveNoteId,
                    {
                        noteTitles: noteTitlesClose,
                        noteColors: noteColorsClose,
                        notePayloads: notePayloadsClose,
                    }
                );
                ...
                const notePayloadsSwitch: Record<
                    string,
                    {
                        content?: string;
                        title?: string;
                ...
                sendNotesUpdateToPip(
                    ...
                    after.openInPipActiveNoteId,
                    {
                        noteTitles: noteTitlesSwitch,
                        noteColors: noteColorsSwitch,
                        notePayloads: notePayloadsSwitch,
                    }
                );
            }
        };
        ...
        >
            ...
            <div
                className={`sidebar-resize-handle ${
                    sidebarCollapsed ? "sidebar-resize-handle--collapsed" : ""
                }`}
                role={sidebarCollapsed ? "button" : "separator"}
                aria-label={
                    sidebarCollapsed ? "Show sidebar" : "Resize sidebar"
                }
                title={
                    sidebarCollapsed
                ...
                onClick={
                    sidebarCollapsed ? handleCollapsedStripClick : undefined
                }
                onKeyDown={
                    sidebarCollapsed ? handleCollapsedStripKeyDown : undefined
                }
            >
                {sidebarCollapsed && (
                    <ChevronRight
                        className="sidebar-resize-handle-icon"
            ...
            >
                {serverNewerBannerVisible && (
                    <div
                        className="server-newer-banner"
                        role="status"
                        aria-live="polite"
                    >
                        <span className="server-newer-banner-text">
                            Data updated on another device. Refresh to get the
                            latest.
                        </span>
                        <button
                            type="button"
                            className="server-newer-banner-refresh"
                            onClick={handleServerNewerRefresh}
                        >
                            Refresh
                        </button>
                    </div>
                ...
                >
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            ...
                            <p className="modal-message">
                                Your session has expired. You have been signed
                                out. Sign in again to sync your notes.
                            </p>
                        </div>
                        <div className="modal-actions">
                            <button
