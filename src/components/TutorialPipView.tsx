...
export function TutorialPipView() {
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      if (d && d.type === 'notic-pip-tutorial-tab-left' && !hasLeftTab) {
        if (import.meta.env.DEV) {
          console.log('[TutorialPiP] User left tab, updating instructions')
        }
        setHasLeftTab(true)
      }
      
      // Listen for signal from main dashboard that user returned to Notic tab
      if (d && d.type === 'notic-pip-tutorial-tab-returned' && !floatTestCompleted) {
        if (import.meta.env.DEV) {
          console.log('[TutorialPiP] Tab returned detected, marking task complete')
        }
        ...
        try {
          if (parentWindow.opener && !parentWindow.opener.closed) {
            parentWindow.opener.postMessage({ type: 'tutorial-task-completed', taskId: 'float-test' }, '*')
            if (import.meta.env.DEV) {
              console.log('[TutorialPiP] Sent completion message to dashboard')
            }
          }
        } catch (e) {
          if (import.meta.env.DEV) {
            console.error('[TutorialPiP] Failed to send completion message:', e)
          }
        }
      }
      
      // Listen for note creation (either toolbar or context menu)
      if (d && d.type === 'notic-pip-tutorial-note-created' && !noteCreated) {
        setNoteCreated(true)
      }
      
      // Listen for note bookmarked
      if (d && d.type === 'notic-pip-tutorial-note-bookmarked' && !noteBookmarked) {
        setNoteBookmarked(true)
      }
      
      // Listen for note opened in PiP (this completes the task!)
      if (d && d.type === 'notic-pip-tutorial-note-opened' && !noteOpened) {
        setNoteOpened(true)
        setShowCelebration(true)
        setTimeout(() => setShowCelebration(false), 3000)
      }
  ...
  }, [floatTestCompleted, hasLeftTab, tabMenuOpened, tabRenamed, tabColorChanged, noteCreated, noteBookmarked, noteOpened])
  ...
  useEffect(() => {
    ...
    try {
      if (parentWindow.opener && !parentWindow.opener.closed) {
        parentWindow.opener.postMessage({ type: 'tutorial-task-completed', taskId: 'tab-customization' }, '*')
        if (import.meta.env.DEV) {
          console.log('[TutorialPiP] Sent tab customization completion message to dashboard')
        }
      }
    } catch (e) {
      if (import.meta.env.DEV) {
    ...
    <>
      <div className="tutorial-pip-container">
        ...
        <div className="pip-tabs">
          <div 
            className={`pip-tab-item active ${tabTaskActive ? 'tutorial-interactive' : ''}`}
            onContextMenu={handleTabContextMenu}
            style={tabTaskActive ? { cursor: 'context-menu' } : undefined}
          >
            {tabColor && (
          ...
          <button type="button" className="pip-tab-new-btn" disabled style={{ opacity: 0.5, cursor: 'not-allowed' }}>
        ...
        <div className="tutorial-pip-content">
          ...
          <ul className="tutorial-pip-task-list">
            <li className={`tutorial-pip-task-item${floatTestCompleted ? ' tutorial-pip-task-checked' : ''}`}>
              <div className={`tutorial-pip-checkbox${floatTestCompleted ? ' tutorial-pip-checkbox-checked' : ''}`}>
                {floatTestCompleted && <Check size={14} strokeWidth={3} />}
              </div>
              <div className="tutorial-pip-task-text">
                <span className="tutorial-pip-task-label">Test the floating window</span>
                <span key={currentInstruction} className="tutorial-pip-task-hint">{currentInstruction}</span>
              </div>
            </li>
            <li className={`tutorial-pip-task-item${tabTaskCompleted ? ' tutorial-pip-task-checked' : ''}${!floatTestCompleted ? ' tutorial-pip-task-disabled' : ''}`}>
              <div className={`tutorial-pip-checkbox${tabTaskCompleted ? ' tutorial-pip-checkbox-checked tutorial-pip-checkbox-pulse' : ''}`}>
                {tabTaskCompleted && <Check size={14} strokeWidth={3} />}
              </div>
              <div className="tutorial-pip-task-text">
                <span className="tutorial-pip-task-label">Customize the tab</span>
                {floatTestCompleted && <span key={tabTaskInstruction} className="tutorial-pip-task-hint">{tabTaskInstruction}</span>}
              </div>
            </li>
            <li className={`tutorial-pip-task-item${noteTaskCompleted ? ' tutorial-pip-task-checked' : ''}${!tabTaskCompleted ? ' tutorial-pip-task-disabled' : ''}`}>
              <div className={`tutorial-pip-checkbox${noteTaskCompleted ? ' tutorial-pip-checkbox-checked tutorial-pip-checkbox-pulse' : ''}`}>
                {noteTaskCompleted && <Check size={14} strokeWidth={3} />}
              </div>
              <div className="tutorial-pip-task-text">
                <span className="tutorial-pip-task-label">Create and open a note</span>
                {tabTaskCompleted && <span key={noteTaskInstruction} className="tutorial-pip-task-hint">{noteTaskInstruction}</span>}
              </div>
            </li>
          </ul>

          <div className="tutorial-pip-note">
            💡 When you create a real note, it will open here with the full editor
          </div>
        </div>
      </div>
      ...
      {contextMenu && tabTaskActive && (
        ...
        >
          ...
          <div
            ...
            onMouseLeave={() => setTimeout(() => setHoveredSubmenu(null), 150)}
          >
      ...
      {showRenameModal && tabTaskActive && (
        <div className="pip-modal-overlay show" onClick={() => setShowRenameModal(false)}>
          <div className="pip-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pip-modal-actions">
              <button type="button" className="pip-modal-btn" onClick={() => setShowRenameModal(false)}>Cancel</button>
              <button type="button" className="pip-modal-btn pip-modal-btn-primary" onClick={handleRenameSave}>Save</button>
            </div>
          </div>
        </div>
      )}
