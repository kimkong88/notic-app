...
import { getFolderNoteCountRecursive, canAcceptFolderDrop } from '../utils/folderUtils'
...
const SHARE_PUBLIC_BASE =
  (typeof import.meta !== 'undefined' && (import.meta.env?.VITE_PUBLIC_URL as string)) || 'https://getnotic.io'
...
function sanitizeSyncLogMessage(message: string): string {
  ...
  const hasRawData =
    ...
    /\b(parentId|folderId|deletedAt|displayName|content|lastModified):\s*/i.test(message)
  if (hasRawData) {
    if (/^Full sync succeeded$/i.test(message.trim())) return message
    if (/^Full sync failed/i.test(message)) return message
    if (/^(Sync|Connecting|Downloading|Uploading)/i.test(message)) return message
    if (/^\s*note\s+"/i.test(message)) return 'Note updated from server'
    if (/^\s*folder\s+"/i.test(message)) return 'Folder updated from server'
    if (/^\s*workspace\s+"/i.test(message)) return 'Workspace updated from server'
    return 'Updated from server'
  }
  const noteMatch = message.match(/^(\s*Note\s+)"([^"]+)"(.*)$/i)
  if (noteMatch) {
    const [, prefix, quoted, suffix] = noteMatch
    const isIdLike = /^[0-9a-f-]{36}$/i.test(quoted) || quoted.length > 40 || /^(clip_|session_)/i.test(quoted)
...
function SyncStatusButton({ setCurrentView: _setCurrentView }: { setCurrentView: (view: 'notes' | 'settings' | 'integrations') => void }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    Promise.all([getLastPullAt(db), getSyncPaused(db)]).then(([ms, paused]) => {
      if (!cancelled) {
  ...
  const displayStatusLabel = syncPaused ? 'Paused' : !isOnline ? 'Offline' : syncStatus === 'synced' ? 'Synced' : syncStatus === 'failed' ? 'Sync failed' : syncStatus === 'syncing' ? 'Syncing…' : 'Local'
  ...
  return (
    ...
    >
      <button
        type="button"
        className={`sync-status sync-status-trigger${syncStatusClass ? ` ${syncStatusClass}` : ''}`}
        id="syncStatus"
      ...
      >
        <StatusIcon className="sync-status-icon" size={12} aria-hidden />
        <span className="sync-status-text">{statusLabel}</span>
        <ChevronDown className="sync-status-chevron" size={12} aria-hidden />
      </button>
      <div
      ...
      >
        <div
          ...
          aria-disabled="true"
        >
          <span className="sync-status-dropdown-label">Last synced</span>
          <span className="sync-status-dropdown-value" id="syncStatusLastSynced">
        ...
        {showPauseResume && (
          <button
            ...
            role="menuitem"
            onClick={(e) => { e.stopPropagation(); void handlePauseResume() }}
          >
        ...
        {isSignedIn && (
          <button
            ...
            className="sync-status-dropdown-row sync-status-dropdown-action"
            role="menuitem"
            onClick={(e) => { e.stopPropagation(); handleAccessSyncLog() }}
          >
            Access sync log
          </button>
        )}
      ...
      {syncLogOpen && (
        <div
          ...
          onKeyDown={(e) => { if (e.key === 'Escape') setSyncLogOpen(false) }}
        >
          <div className="modal modal-sync-log" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 id="sync-log-title" className="modal-title">Sync log</h3>
              <p className="modal-message">Recent sync events for this device.</p>
            </div>
            <div className="modal-folder-picker-list scrollbar-branded modal-sync-log-list">
              {syncLogEntries.length === 0 ? (
                <div className="modal-sync-log-empty">No sync events yet.</div>
              ) : (
                [...syncLogEntries]
                  .sort((a, b) => b.at - a.at)
                  .map((e) => (
                    <div key={`${e.at}-${e.message}`} className="modal-sync-log-item">
                      <span className="modal-sync-log-time">{formatDate(e.at)}</span>
                      <span className="modal-sync-log-message">{sanitizeSyncLogMessage(e.message)}</span>
                    </div>
                  ))
              )}
            </div>
      ...
      {syncLimitModalOpen && (
        ...
        >
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 id="sync-limit-title" className="modal-title">Sync limit reached</h3>
              <p className="modal-message">
                Notic is now running in Private Local Mode to protect your data. Upgrade to Pro to resume Cloud Sync for all {Object.keys(notes).length} notes, or delete notes from Trash to free sync space.
              </p>
            </div>
            <div className="modal-actions">
              <button
...
function getFolderPath(folderId: string | null, folders: Record<string, Folder>): Array<{ id: string; name: string }> {
  ...
  return [...notes].sort((a, b) => {
    switch (sort) {
      ...
      case 'alphabetical-desc':
        return (b.displayName || b.title).localeCompare(a.displayName || a.title)
      default:
        return b.lastModified - a.lastModified
    }
  })
...
export function MainContent() {
  ...
  useEffect(() => {
    if (selectedNoteId != null && detailEditNoteId != null && detailEditNoteId !== selectedNoteId) {
  ...
  useEffect(() => {
    ...
    if (!container) return
    const contentEl = container.querySelector('.note-editor-content-editable') as HTMLElement | null
    if (!searchQuery.trim() || !selectedNoteId || detailEditNoteId === selectedNoteId) {
    ...
    return () => {
      ...
      const el = c?.querySelector('.note-editor-content-editable') as HTMLElement | null
  ...
  const openNoteInPip = (note: { sessionId: string; title?: string; content?: string }) => {
  ...
  type FolderViewItem = { type: 'folder'; data: Folder } | { type: 'note'; data: NoteData }
  const folderViewItems = useMemo((): FolderViewItem[] => {
    ...
    if (
      ...
      return [
        ...
        ...notesInFolder.map((n) => ({ type: 'note' as const, data: n })),
      ]
    }
    ...
    return [
      ...
      ...sortedFolderless.map((n) => ({ type: 'note' as const, data: n })),
    ]
  }, [
    currentTab,
    selectedFolderId,
  ...
  const handleNoteDetailContentChange = useCallback(
    (content: string) => {
      ...
      if (saveNoteContentDebounceRef.current) clearTimeout(saveNoteContentDebounceRef.current)
  ...
  const clearMainListDragOver = useCallback(() => {
    document.querySelectorAll('.main-notes-list .drag-over').forEach((el) => el.classList.remove('drag-over'))
  }, [])

  const handleNoteDragStart = useCallback((e: React.DragEvent, sessionId: string) => {
    if (e.dataTransfer) {
  ...
  const handleFolderRowDragOver = useCallback(
    (e: React.DragEvent, targetFolderId: string) => {
      ...
      if (types.includes(FOLDER_DROP_TYPE)) {
        ...
        if (draggedId && canAcceptFolderDrop(draggedId, targetFolderId, folders)) {
          e.preventDefault()
          e.stopPropagation()
  ...
  const handleFolderRowDrop = useCallback(
    (e: React.DragEvent, targetFolderId: string) => {
      ...
      if (folderId && canAcceptFolderDrop(folderId, targetFolderId, folders)) {
  ...
  const breadcrumbItems = useMemo((): BreadcrumbItem[] => {
    ...
    if (isTrashView) {
      items.push({ text: 'Trash', active: !selectedNoteId, click: 'trash' })
      if (selectedNoteId && selectedNote) {
        items.push({
          text: `${(selectedNote.displayName ?? selectedNote.title) || 'Untitled'}.md`,
          active: true,
          click: 'note',
          id: selectedNoteId,
        })
    ...
    if (currentTab === 'recent') {
      if (searchQuery.trim()) {
        ...
        if (selectedFolderDate === BOOKMARKS_SENTINEL) {
          if (selectedNoteId && selectedNote) {
            items.push({ text: 'Bookmarks', active: false, click: 'bookmarks', id: BOOKMARKS_SENTINEL })
            items.push({
              text: `${stripMarkdownForDisplay((selectedNote.displayName ?? selectedNote.title) || 'Untitled')}.md`,
              active: true,
              click: 'note',
              id: selectedNoteId,
            })
          } else {
            items.push({ text: 'Bookmarks', active: true, click: 'bookmarks', id: BOOKMARKS_SENTINEL })
          }
        } else {
          const formattedDate = formatBreadcrumbDate(selectedFolderDate)
          if (selectedNoteId && selectedNote) {
            items.push({ text: formattedDate, active: false, click: 'date', id: selectedFolderDate })
            items.push({
              text: `${stripMarkdownForDisplay((selectedNote.displayName ?? selectedNote.title) || 'Untitled')}.md`,
              active: true,
              click: 'note',
            ...
            items.push({ text: formattedDate, active: true, click: 'date', id: selectedFolderDate })
          }
        }
      } else {
      ...
      } else if (selectedFolderId === ROOT_SENTINEL || selectedFolderId === null) {
        if (selectedNoteId && selectedNote) {
          items.push({ text: 'Root', active: false, click: 'folder', id: ROOT_SENTINEL })
          items.push({
            text: `${(selectedNote.displayName ?? selectedNote.title) || 'Untitled'}.md`,
            active: true,
            click: 'note',
          ...
          items.push({ text: 'Root', active: true, click: 'folder', id: ROOT_SENTINEL })
        }
      } else if (selectedFolderId) {
        const path = getFolderPath(selectedFolderId, folders)
        if (path.length > 0) {
          path.forEach((p, i) => {
            const isLastFolder = i === path.length - 1 && !selectedNoteId
            items.push({ text: p.name, active: isLastFolder, click: 'folder', id: p.id })
          })
          if (selectedNoteId && selectedNote) {
            if (items.length > 0) items[items.length - 1].active = false
            items.push({
              text: `${stripMarkdownForDisplay((selectedNote.displayName ?? selectedNote.title) || 'Untitled')}.md`,
              active: true,
              click: 'note',
      ...
      <main className="main-content">
            ...
            {breadcrumbItems.map((item, index) => (
              <Fragment key={index}>
                ...
                {item.click != null ? (
                  <button
                    type="button"
                    className={`breadcrumb-item breadcrumb-item-clickable${item.active ? ' active' : ''}`}
                    data-crumb={item.click}
                    data-crumb-id={item.id}
                    title={item.text}
                    onClick={() => handleBreadcrumbClick(item.click!, item.id)}
                  >
                    {item.text}
                  </button>
                ) : (
                  <span className={`breadcrumb-item${item.active ? ' active' : ''}`} title={item.text}>
                    {item.text}
                  </span>
                )}
              </Fragment>
        ...
        {currentView === 'notes' && (
          <div className={`content-view${isTrashView ? ' trash-view-active' : ''}`} id="notesView">
            ...
            <div className="notes-content" id="notesContent">
              {!isTrashView && allNotesInWorkspace.length === 0 && !searchQuery.trim() ? (
                ...
                <div className="settings-page">
                  <div className="settings-page-content">
                    <div className="trash-header-row">
                      <h2 className="settings-page-title">Trash</h2>
                    ...
                    {trashNotesInWorkspace.length === 0 ? (
                      <p className="trash-empty-desc">Trash is empty</p>
                    ) : (
                      <div className="trash-list">
                        {trashNotesInWorkspace.map((note) => {
                          const displayName = note.displayName || note.title || 'Untitled'
                          return (
                            <div key={note.sessionId} className="note-item" data-session-id={note.sessionId}>
                              <div className="note-header">
                                <div className="note-title">
                                  ...
                                  <FileText className="note-file-icon" size={16} aria-hidden />
                        ...
                        <div className="note-detail-header-main">
                          <div className="note-detail-title-row">
                            ...
                            <h1 className="note-detail-title">
                              {stripMarkdownForDisplay(selectedNote.displayName || selectedNote.title || 'Untitled')}.md
                            </h1>
                          </div>
                          <p className="note-detail-meta-inline">
                            {formatDate(selectedNote.lastModified)} · {selectedNote.wordCount} words
                          </p>
                        </div>
                        <div className="note-detail-actions">
                          {isEditMode ? (
                            <button
                              ...
                              onClick={() => setDetailEditNoteId(null)}
                            >
                              <Check size={16} />
                            </button>
                          ) : (
                          ...
                          <button
                            type="button"
                            className={`note-detail-action-btn ${selectedNote.shareCode ? 'note-detail-action-shared' : ''}`}
                            title={selectedNote.shareCode ? 'Shared' : 'Share'}
                            aria-label={selectedNote.shareCode ? 'Shared' : 'Share'}
                            onClick={() => setShareModalNoteId(selectedNoteId!)}
                          >
                            <Share2 size={16} />
                          </button>
                          <button
                ...
                filteredAndSortedNotes.length === 0 ? (
                  <div className="empty-state-main">
                    <p>No notes found matching &quot;{searchQuery.trim()}&quot;</p>
                  </div>
                ) : (
                  <div className="main-notes-list">
                    {filteredAndSortedNotes.map((note) => {
                      ...
                      return (
                        <div
                          key={note.sessionId}
                          className={`note-item ${selectedNoteId === note.sessionId ? 'note-item-selected' : ''} ${isOpenInPip ? 'note-item-open-in-pip' : ''}`}
                          onClick={() => handleRowClick(note)}
                          onContextMenu={(e) => {
                            e.preventDefault()
                            setNoteContextMenuAnchor({ x: e.clientX, y: e.clientY, noteId: note.sessionId })
                          }}
                          role="button"
                          tabIndex={0}
                          onKeyDown={(e) => {
                              ...
                              {searchQuery.trim() ? (
                                <span
                                  dangerouslySetInnerHTML={{
                                    __html: highlightMatch(escapeHtml(`${stripMarkdownForDisplay(displayName)}.md`), searchQuery.trim()),
                                  }}
                                />
                              ) : (
                                <span>{stripMarkdownForDisplay(displayName)}.md</span>
                              )}
                            </div>
                            <button
                              type="button"
                              className={`note-pip-icon ${shouldDisable ? 'disabled' : ''}`}
                              onClick={(e) => handlePipIconClick(e, note)}
                              title={
                                shouldDisable
                                  ? 'Note is already open in editor'
                                  : 'Open editor'
                              ...
                              <span
                                dangerouslySetInnerHTML={{
                                  __html: highlightMatch(
                                    escapeHtml(
                                      note.content ? getContentPreview(note.content, 150) : ''
                                    ),
                                    searchQuery.trim()
                                  ),
                                }}
              ...
              ) : currentTab === 'folders' && !searchQuery.trim() && !isTrashView ? (
                    ...
                    {folderViewItems.map((item) =>
                      item.type === 'folder' ? (
                        <div
                          ...
                          draggable
                          onClick={() => handleFolderRowClick(item.data.id)}
                          onDragStart={(e) => handleFolderDragStart(e, item.data.id)}
                          onDragEnd={handleFolderDragEnd}
                          onDragOver={(e) => handleFolderRowDragOver(e, item.data.id)}
                          onDragLeave={handleFolderRowDragLeave}
                          onDrop={(e) => handleFolderRowDrop(e, item.data.id)}
                          role="button"
                          tabIndex={0}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                              e.preventDefault()
                              handleFolderRowClick(item.data.id)
                            }
                          }}
                        >
                          <div className="note-header">
                            <div className="note-title">
                              ...
                              <FolderIcon className="folder-icon" size={16} aria-hidden />
                              <span>{item.data.displayName ?? item.data.name}</span>
                            </div>
                          </div>
                          <div className="note-metadata">
                            <span>
                        ...
                        <div
                          ...
                          className={`note-item ${selectedNoteId === item.data.sessionId ? 'note-item-selected' : ''} ${openInPipNoteIds.includes(item.data.sessionId) ? 'note-item-open-in-pip' : ''}`}
                          draggable
                          onClick={() => handleRowClick(item.data)}
                          onContextMenu={(e) => {
                            e.preventDefault()
                            setNoteContextMenuAnchor({ x: e.clientX, y: e.clientY, noteId: item.data.sessionId })
                          }}
                          onDragStart={(e) => handleNoteDragStart(e, item.data.sessionId)}
                        ...
                        >
                          <div className="note-header">
                            <div className="note-title">
                              ...
                              <FileText className="note-file-icon" size={16} aria-hidden />
                              <span>
                                {stripMarkdownForDisplay(item.data.displayName ?? item.data.title ?? 'Untitled')}.md
                              </span>
                            </div>
                            <button
                              type="button"
                          ...
                          <div className="note-content">
                            {item.data.content
                              ? getContentPreview(item.data.content, 150)
                              : ''}
                          </div>
                          <div className="note-metadata">
                            <span>{formatDate(item.data.lastModified)}</span>
                            <span className="note-metadata-separator" />
                            <span>{item.data.wordCount} words</span>
                          </div>
                        </div>
                      )
                    )}
              ...
              ) : currentTab === 'recent' && !selectedFolderDate ? (
                <div className="empty-state-main">
                  <p>Select a folder or note to view</p>
                </div>
              ) : currentTab === 'folders' && selectedFolderId === null && filteredAndSortedNotes.length === 0 ? (
                <div className="empty-state-main">
                ...
                <div className="empty-state-main">
                  <p>
                    {(currentTab === 'recent' && selectedFolderDate === BOOKMARKS_SENTINEL) ||
                    (currentTab === 'folders' && selectedFolderId === BOOKMARKS_SENTINEL)
                        ...
                        <div className="note-header">
                          <div className="note-title">
                            ...
                            {searchQuery.trim() ? (
                              <span
                                dangerouslySetInnerHTML={{
                                  __html: highlightMatch(escapeHtml(`${displayName}.md`), searchQuery.trim()),
                                }}
                              />
                            ) : (
                              <span>{displayName}.md</span>
        ...
        {shareModalNoteId != null && notes[shareModalNoteId] != null && (() => {
          ...
          return (
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Escape') setShareModalNoteId(null) }}
            >
              <div className="modal modal-share" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                  <h3 id="share-modal-title" className="modal-title">Share</h3>
                  <p className="modal-message">Anyone with the link can view this note (read-only).</p>
                  {published && (
                    <div className="share-link-row">
                      ...
                      <button
                        ...
                        title="Copy link"
                        onClick={() => {
                          navigator.clipboard.writeText(shareUrl).then(() => setShareToast('Link copied'))
                        }}
                      >
                ...
                <div className="modal-actions">
                  {!published ? (
                    <button
                      ...
                      onClick={async () => {
                        if (!isSignedIn) {
                          setShareToast('Sign in to share notes')
                          return
                        }
                        setSharePublishLoading(true)
                        const result = await publishNote(db, shareModalNoteId)
                        if (result && 'paymentRequired' in result && result.paymentRequired) {
                          setProRequiredModal({
                            title: 'Pro required',
                            message: 'Publish to web is a Pro feature. Upgrade to share notes with a public link.',
                          })
                          setSharePublishLoading(false)
                          return
                        }
                        if (result && 'shareCode' in result && typeof result.shareCode === 'string') {
                          updateNote(shareModalNoteId, { shareCode: result.shareCode })
                          setSharePublishLoading(false)
                        } else {
                          setShareToast('Could not publish. Sign in and try again.')
                          setSharePublishLoading(false)
                        }
                      }}
                    >
                      {sharePublishLoading ? 'Publishing…' : 'Publish to web'}
                    </button>
                  ) : (
                    <>
                      <button
                      ...
                      <button
                        type="button"
                        className="modal-btn modal-btn-secondary"
                        onClick={() => setShareUnpublishConfirm(true)}
                      >
                        Unpublish
                      </button>
                    </>
        ...
        {shareUnpublishConfirm && shareModalNoteId != null && (
          <div
            ...
            onKeyDown={(e) => { if (e.key === 'Escape') setShareUnpublishConfirm(false) }}
          >
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h3 id="unpublish-confirm-title" className="modal-title">Unpublish note</h3>
                <p className="modal-message">The link will stop working. You can publish again anytime.</p>
              </div>
              <div className="modal-actions">
                <button
                  type="button"
                  ...
                  onClick={async () => {
                    ...
                    const result = await unpublishNote(db, noteId)
                    if (result?.paymentRequired) {
                      setProRequiredModal({
                        title: 'Pro required',
                        message: 'Publish to web is a Pro feature. Upgrade to share and unpublish notes.',
                      })
                      return
                    }
                    if (result?.ok) {
                      updateNote(noteId, { shareCode: undefined })
                      setShareModalNoteId(null)
                    } else {
                      setShareToast('Could not unpublish.')
                    }
                  }}
                >
                  Unpublish
        ...
        {proRequiredModal && (
          <div
            ...
            onKeyDown={(e) => { if (e.key === 'Escape') setProRequiredModal(null) }}
          >
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h3 id="pro-required-title" className="modal-title">{proRequiredModal.title}</h3>
