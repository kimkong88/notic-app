...
export function SettingsView() {
    ...
    const [noteTheme, setNoteThemeState] =
        useState<NoteThemeId>(getStoredNoteTheme);
    ...
    const handleExportZip = useCallback(async () => {
        ...
        try {
            ...
            const name = `notic-export-${new Date()
                .toISOString()
                .slice(0, 10)}.zip`;
        ...
        async (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            try {
                ...
                if (result.notesImported === 0 && result.foldersCreated === 0) {
                    ...
                    const folderWord =
                        result.foldersCreated === 1 ? "folder" : "folders";
                    const parts = [
                        `${result.notesImported} ${noteWord} imported.`,
                    ];
                    if (result.foldersCreated > 0) {
                        parts.push(
                            `${result.foldersCreated} ${folderWord} created.`
                        );
                    }
                    if (result.skipped > 0) {
    ...
    const handleNotionConnect = useCallback(async () => {
        ...
        setToastMessage(
            "Complete the connection in the opened tab, then return here."
        );
    }, [authUser]);
    ...
    const handleNotionSetSyncRoot = useCallback(async () => {
        ...
        try {
            ...
            if (ok) {
                ...
                setToastMessage(
                    "Failed to set sync page. Check the URL or page ID."
                );
            }
        } finally {
    ...
    const handleSyncToNotion = useCallback(async () => {
        ...
        try {
            ...
            if (result.paymentRequired) {
                setToastMessage(
                    "Notion sync is a Pro feature. Upgrade at getnotic.io/billing."
                );
                return;
            }
            if (result.ok) {
    ...
    const handleObsidianExport = useCallback(async () => {
        ...
        try {
            ...
            if (data && "paymentRequired" in data && data.paymentRequired) {
                setToastMessage(
                    "Obsidian export is a Pro feature. Upgrade at getnotic.io/billing."
                );
                return;
            }
            ...
            const name = `notic-obsidian-export-${new Date()
                .toISOString()
                .slice(0, 10)}.zip`;
            downloadExportBlob(blob, name);
            trackEvent("export_completed", { format: "obsidian" });
            setToastMessage(
                `Exported ${data.files.length} file(s). Extract the ZIP into your Obsidian vault.`
            );
        } catch (e) {
    ...
    const handleInstall = useCallback(async () => {
        ...
        try {
            ...
            if (result.outcome === "accepted") {
                ...
                setToastMessage(
                    "Notic installed! Check your desktop or app menu."
                );
                trackEvent("pwa_installed", { source: "settings" });
            }
        } catch (e) {
    ...
    if (settingsSubView === "integrations") {
        const notionStatusText = notionStatusLoading
            ? "Loading…"
            : !authUser
            ...
            : (() => {
                  const parts: string[] = [
                      `Connected to ${
                          notionStatus.notionWorkspaceName ?? "Notion"
                      }.`,
                  ];
        ...
        const notionSyncRootPlaceholder = notionStatus?.syncRootPageId
            ? `Sync root set (ID: …${
                  notionStatus.syncRootPageId?.slice(-8) ?? ""
              })`
                ...
                <div className="settings-page-content">
                    <div className="settings-integrations-header">
                        ...
                        <p className="settings-integrations-intro">
                            Sync to Notion or export your notes to Obsidian.
                        </p>
                    </div>
                    <section className="settings-section settings-section-notion">
                        <div className="settings-notion-header">
                            <img
                                src={NOTION_ICON_URL}
                                alt=""
                                className="settings-notion-section-icon"
                                aria-hidden
                            />
                        ...
                        <ol className="settings-notion-steps">
                            <li className="settings-notion-step">
                                <span
                                    className="settings-notion-step-num"
                                    aria-hidden
                                >
                                    1
                                </span>
                                <div className="settings-notion-step-body">
                                    <strong className="settings-notion-step-title">
                                        Connect your account
                                    </strong>
                                    <p className="settings-notion-step-desc">
                                        Opens Notion in a new tab so you can
                                        authorize. You only need to do this
                                        once.
                                    </p>
                                    <div className="settings-notion-status">
                                        {notionStatusText}
                                    </div>
                                    <div className="settings-notion-actions">
                                        ...
                                        >
                                            <img
                                                src={NOTION_ICON_URL}
                                                alt=""
                                                className="settings-notion-btn-icon"
                                                aria-hidden
                                            />
                            ...
                            <li
                                className={`settings-notion-step ${
                                    showNotionSteps2And3
                                        ? ""
                                        : "content-view-hidden"
                                }`}
                            >
                                ...
                                <div className="settings-notion-step-body">
                                    ...
                                    <p className="settings-notion-step-desc">
                                        Paste the full Notion page URL (e.g.{" "}
                                        <code>
                                            https://notion.so/My-Page-abc123...
                                        </code>
                                        ) or just the page ID. Notes will be
                                        pushed under this page.
                                    </p>
                            ...
                            <li
                                className={`settings-notion-step settings-notion-step-sync ${
                                    showNotionSteps2And3
                                        ? ""
                                        : "content-view-hidden"
                                }`}
                            >
                                ...
                                <div className="settings-notion-step-body">
                                    ...
                                    <p className="settings-notion-step-desc">
                                        Sends your current workspace notes to
                                        the page you set above. Run this
                                        whenever you want to update Notion.
                                    </p>
                                    <button
                                        type="button"
                    ...
                    <section className="settings-section settings-section-obsidian">
                        <div className="settings-obsidian-header">
                            <img
                                src={OBSIDIAN_ICON_URL}
                                alt=""
                                className="settings-obsidian-section-icon"
                                aria-hidden
                            />
                            <h4 className="settings-section-title">Obsidian</h4>
                        </div>
                        <p className="settings-section-desc">
                            Export all your notes as Markdown files. Download a
                            ZIP and extract it into your Obsidian vault.
                        </p>
                        <div className="settings-obsidian-how">
                            <strong className="settings-obsidian-how-title">
                                How to use with Obsidian
                            </strong>
                            <ol className="settings-obsidian-steps">
                                <li>
                                    Click <strong>Export to Obsidian</strong> to
                                    download a ZIP of your notes.
                                </li>
                                <li>
                                    Extract the ZIP into your Obsidian vault
                                    folder (or a folder inside it).
                                </li>
                                <li>
                                    Notes are <code>.md</code> files with the
                                    same workspace and folder structure.
                                    Re-export anytime to update.
                                </li>
                            </ol>
                        </div>
                        <button
        ...
        <div className="settings-page">
            <div className="settings-page-content">
                ...
                <section className="settings-section settings-section-appearance">
                    ...
                    <p className="settings-section-desc">
                        App theme and note editor look.
                    </p>
                    <div className="settings-field">
                        ...
                        >
                            <label className="settings-app-option">
                                ...
                                />
                                <span
                                    className="settings-app-preview settings-app-preview-light"
                                    aria-hidden
                                />
                            ...
                            <label className="settings-app-option">
                                ...
                                />
                                <span
                                    className="settings-app-preview settings-app-preview-dark"
                                    aria-hidden
                                />
                    ...
                    <div className="settings-field">
                        ...
                        <div
                            className="settings-note-theme-grid"
                            role="radiogroup"
                        ...
                        >
                            {NOTE_THEME_OPTIONS.map((opt) => (
                                <label
                                    key={opt.value}
                                    className="settings-note-option"
                                >
                                    <input
                                        type="radio"
                                        name="noteTheme"
                                    ...
                                    />
                                    <span
                                        className={`settings-note-preview settings-note-preview-${
                                            opt.value === "default"
                                                ? "default"
                                    ...
                                    <span className="settings-note-name">
                                        {opt.label}
                                    </span>
                                </label>
                            ))}
                ...
                <section className="settings-section settings-section-workspace-appearance">
                    ...
                    <p className="settings-section-desc">
                        Icon and color for the current workspace (shown in the
                        sidebar).
                    </p>
                    {currentWorkspace ? (
                        <>
                            ...
                            <div className="settings-field">
                                ...
                                <div
                                    className="settings-workspace-colors"
                                    role="radiogroup"
                                ...
                                >
                                    {[
                                        ...
                                        <button
                                            key={value || "default"}
                                            type="button"
                                            className={`settings-workspace-color-btn ${
                ...
                <section className="settings-section settings-section-plan">
                    ...
                    <p className="settings-section-desc">
                        {authUser
                            ? "Manage your Notic plan and billing."
                            : "Sign in to view and manage your Notic plan and billing."}
                    </p>
                ...
                <section className="settings-section settings-section-export">
                    ...
                    <p className="settings-section-desc">
                        Download all notes in this workspace as a ZIP of
                        Markdown files, with folder structure preserved. Export
                        your data anytime. Notes are stored locally and synced
                ...
                <section className="settings-section settings-section-import">
                    ...
                    <p className="settings-section-desc">
                        Import notes from a ZIP of Markdown files. Only .md
                        files are imported; folder structure is recreated.
                    </p>
                    <input
                        ref={importInputRef}
                        type="file"
                ...
                <section className="settings-section settings-section-about">
                    ...
                    <p className="settings-section-desc">
                        Notic — minimalist notes with a floating editor.
                    </p>
                    <div className="settings-about-row">
                        <span className="settings-about-version">Web app</span>
                        <a
                            href="https://getnotic.io"
                            target="_blank"
                    ...
                    <div className="settings-about-links">
                        <a
                            href="https://getnotic.io/privacy"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="settings-about-link"
                        >
                        ...
                        <span className="settings-about-sep" aria-hidden>
                            ·
                        </span>
                        <a
                            href="https://getnotic.io/terms"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="settings-about-link"
                        >
