...
describe("useNotesStore", () => {
    ...
    describe("toggleFolderExpanded", () => {
        ...
        it("tracks main folder expansion separately from sidebar", () => {
            ...
            expect(useNotesStore.getState().expandedMainFolderIds).toEqual([
                "f1",
            ]);
        });
    });
    ...
    describe("duplicateNote", () => {
        it("returns null when note does not exist", () => {
            expect(
                useNotesStore.getState().duplicateNote("missing")
            ).toBeNull();
        ...
        it("creates a clone with new sessionId and (copy) in title/displayName", () => {
            ...
            expect(clone.createdAt).toBeGreaterThanOrEqual(
                clone.lastModified - 1
            );
    ...
    describe("setSearchQuery (search: list only, save/restore selection)", () => {
        it("when entering search: saves selectedSidebarContext and selectedNoteId, then clears selection", () => {
            ...
            expect(useNotesStore.getState().previousSidebarContext).toBe(
                "2025-02-01"
            );
        ...
        it("when exiting search: restores selectedSidebarContext and selectedNoteId, clears previous", () => {
            ...
            expect(useNotesStore.getState().selectedSidebarContext).toBe(
                "2025-02-01"
            );
    ...
    describe("delete navigates back to parent view", () => {
        const note = {
            sessionId: "n1",
            content: "Hello",
            lastModified: 100,
        ...
        it("removeNote clears selectedNoteId when the removed note is selected", () => {
            useNotesStore.getState().setNotes({ n1: note });
            useNotesStore.getState().setSelectedNoteId("n1");
            expect(useNotesStore.getState().selectedNoteId).toBe("n1");
            useNotesStore.getState().removeNote("n1");
            expect(useNotesStore.getState().selectedNoteId).toBeNull();
            expect(useNotesStore.getState().notes).not.toHaveProperty("n1");
        });

        it("removeNote preserves selectedNoteId when a different note is removed", () => {
            const note2 = { ...note, sessionId: "n2" };
            useNotesStore.getState().setNotes({ n1: note, n2: note2 });
            useNotesStore.getState().setSelectedNoteId("n1");
            useNotesStore.getState().removeNote("n2");
            expect(useNotesStore.getState().selectedNoteId).toBe("n1");
        });

        it("updateNote with deletedAt (soft-delete) does NOT auto-clear selectedNoteId", () => {
            // The component is responsible for clearing selectedNoteId after soft-delete
            useNotesStore.getState().setNotes({ n1: note });
            useNotesStore.getState().setSelectedNoteId("n1");
            useNotesStore
                .getState()
                .updateNote("n1", { deletedAt: Date.now() });
            // Store does NOT clear it — component must call setSelectedNoteId(null)
            expect(useNotesStore.getState().selectedNoteId).toBe("n1");
            expect(useNotesStore.getState().notes.n1.deletedAt).toBeDefined();
        });

        it("restoreNote keeps selectedNoteId unchanged", () => {
            const deletedNote = { ...note, deletedAt: Date.now() };
            useNotesStore.getState().setNotes({ n1: deletedNote });
            useNotesStore.getState().setSelectedNoteId("n1");
            useNotesStore.getState().restoreNote("n1");
            expect(useNotesStore.getState().selectedNoteId).toBe("n1");
            expect(useNotesStore.getState().notes.n1.deletedAt).toBeUndefined();
        });
    });
    ...
    describe("setExpandedSidebarFolderIds", () => {
        it("replaces expandedSidebarFolderIds with given array", () => {
            ...
            useNotesStore
                .getState()
                .setExpandedSidebarFolderIds(["f1", "f2", "f3"]);
            expect(useNotesStore.getState().expandedSidebarFolderIds).toEqual([
                "f1",
                "f2",
                "f3",
