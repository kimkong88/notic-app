...
interface UIActions {
  ...
  setNoteContextMenuAnchor: (anchor: { x: number; y: number; noteId: string } | null) => void;
...
export const useUIStore = create<UIState & UIActions>((set, get) => ({
  ...
  setCurrentView: (view) =>
    set((_s) => ({
      currentView: view,
      ...(view === 'settings' ? { settingsSubView: 'main' as const } : {}),
    })),
  setSettingsSubView: (sub) => set({ settingsSubView: sub }),
  setIsTrashView: (value) => set({ isTrashView: value }),
  setOpenInPipNoteIds: (ids) => set({ openInPipNoteIds: ids, openInPipActiveNoteId: ids[0] ?? null }),
  ...
  addNoteToPip: (noteId, setActive = true) => {
    ...
    set({ openInPipNoteIds: next, openInPipActiveNoteId: setActive ? noteId : (next[0] ?? null) })
  },
  ...
  setPipUnsupportedModalOpen: (value) => set({ pipUnsupportedModalOpen: value }),
  setDetailEditNoteId: (id) => set({ detailEditNoteId: id }),
  setNoteContextMenuAnchor: (anchor) => set({ noteContextMenuAnchor: anchor }),
  setShareModalNoteId: (id) => set({ shareModalNoteId: id }),
  setSessionExpiredModalOpen: (value) => set({ sessionExpiredModalOpen: value }),
